...
| Memes page | ✅ | |
| Centro de Canales (multi-cuenta publish) | ✅ | Gated por switch en Ajustes (`channels_feature_enabled`, OFF default). Multi-cuenta YouTube/TikTok/Instagram (OAuth login navegador, tokens cifrados Fernet por-cuenta). Cola async + programación (`PublishJob`, scheduler tick), batch episodio+shorts, historial. YouTube end-to-end; TikTok/IG código listo, encienden con app-creds del usuario. Página `/channels` + panel publicar en EpisodePage. 20 tests + smoke HTTP |
| Recreations page | ✅ | **Final Alternativo (i2v)**: ingesta (drag-drop mp4 + YouTube vía yt-dlp tras gate de derechos) → conserva head original [0:T] → frame@T → `image_to_video` (continuación) → concat. Es i2v, NO v2v (v2v re-renderizaría el head; además `video_to_video` no tiene handler/proveedor). Engine `AlternateEndingService` con i2v inyectable: 6 tests ffmpeg hermético + 6 ingesta + smoke endpoints. El paso i2v es el único de pago (sin verificar end-to-end por la regla no-generar). `run_recreation` (text_to_video stub) queda legacy; el flujo nuevo vive en `/brain/recreations/{ingest,alternate-ending}` + pestaña "Final alternativo" |
