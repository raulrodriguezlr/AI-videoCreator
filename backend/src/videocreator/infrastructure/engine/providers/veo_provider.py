...
class VeoProvider(BaseVideoProvider):
    ...
    def _handle_scene_error(
        ...
        """
        error_msg = str(error)
        log.error("veo.scene_error", scene=scene_num, error=error_msg[:120])

        if is_rate_limit_error(error):
            if self.key_manager.rotate_key(error_msg):
                self.client = self.key_manager.get_client()
                log.info("veo.retrying_rotated_key", scene=scene_num, key=self.key_manager.get_key_label(), active_key=self.key_manager.get_active_key())
                try:
                    ...
                    log.error("veo.retry_failed", error=error_msg[:120])

            # All keys exhausted
            log.error("veo.all_keys_exhausted")
            if progress_manager:
                progress_manager.mark_scene_failed(i, error_msg, is_rate_limit=True)
            return {"rate_limited": True}
        else:
            # Non-retriable error
            log.error("veo.non_retriable_error", scene=scene_num)
            if progress_manager:
