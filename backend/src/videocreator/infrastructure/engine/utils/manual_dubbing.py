...
class ManualDubber:
    ...
    def _dub_single_clip(self, clip_path: str, scene_data: dict, clips_dir: str, scene_num: int) -> Optional[str]:
        ...
        # 2. Separar voz de efectos (Demucs) y aplicar STS
        ...
        if separated:
            ...
            print(f"[DUBBING] ⚠️ Demucs no disponible o falló, usando STS sobre audio bruto...")
            converted_audio = self.eleven_prov.convert_voice(
        ...
        # Mezclar con SFX original si se separó
        if converted_audio and sfx_track and os.path.exists(sfx_track):
            remixed_path = audio_path.replace(".wav", "_remixed.wav")
            remixed = AudioSeparator.remix_voice_with_sfx(
                dubbed_voice_path=converted_audio,
                sfx_path=sfx_track,
    ...
    def _dub_all_clips(self, available_clips: List[str], scenes: List[dict], ep_dir: str, clips_dir: str):
        ...
        # 2. Separar voz de efectos y convertir STS global
        ...
        if AudioSeparator.is_available():
            print("🔄 Separando voces y SFX globales con Demucs...")
            separated = AudioSeparator.separate(
                audio_path=veo_audio_path,
        ...
        if separated:
            ...
            print("🔄 Convirtiendo voz aislada con ElevenLabs STS...")
            converted_audio = self.eleven_prov.convert_voice(
                source_audio_path=vocals_path,
                character_name=character_name,
                output_path=audio_path
            ...
            print("🔄 Convirtiendo voz de pista combinada con ElevenLabs STS...")
            converted_audio = self.eleven_prov.convert_voice(
                source_audio_path=veo_audio_path,
                character_name=character_name,
                output_path=audio_path
        ...
        if converted_audio and sfx_track and os.path.exists(sfx_track):
            remixed_path = audio_path.replace(".wav", "_remixed.wav")
            print("🎛️ Remezclando pista de voz doblada con efectos SFX globales...")
            remixed = AudioSeparator.remix_voice_with_sfx(
                dubbed_voice_path=converted_audio,
                sfx_path=sfx_track,
                output_path=remixed_path,
            ...
            if remixed:
                converted_audio = remixed
                print(f"[DUBBING] 🔊 Voz global doblada + efectos originales remezclados")
            try:
                os.remove(sfx_track)
            except OSError:
                pass
        ...
        print("\n🎛️ Mezclando pista de audio final con el vídeo global...")
        final_clip_path = AudioMixer.mix_audio_to_video(
