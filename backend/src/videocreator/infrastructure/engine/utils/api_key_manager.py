...
Uso:
    from videocreator.infrastructure.engine.utils.api_key_manager import api_key_manager
...
class ApiKeyManager:
    ...
    def __init__(self):
        ...
        log.info("api_keys_loaded", count=len(self.keys))
    ...
    def rotate_key(self, error_msg: str = "") -> bool:
        ...
        if next_index == 0 and old_index > 0:
            log.error("all_api_keys_exhausted", total_keys=len(self.keys))
            return False
        elif len(self.keys) == 1:
            log.error("api_key_single_exhausted")
            return False
        ...
        log.warning("api_key_rotated", from_key=old_index + 1, to_key=self.current_index + 1)
        return True
