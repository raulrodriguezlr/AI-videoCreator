...
class SqlUserRepository(SqlBase):
    ...
    async def save(self, user: User) -> User:
        async with self._session() as session:
            ...
            if existing is None:
                session.add(
                    UserRow(id=user.id, email=user.email, role=user.role, created_at=user.created_at)
                )
            else:
...
class SqlTopicRepository(SqlBase):
    ...
    async def list_for_pod(self, pod_id: PodId) -> list[Topic]:
        async with self._session() as session:
            result = await session.execute(
                select(TopicRow).where(TopicRow.pod_id == pod_id).order_by(TopicRow.created_at.desc())
            )
    ...
    async def save(self, topic: Topic) -> Topic:
        async with self._session() as session:
            ...
            if existing is None:
                ...
                existing.status = topic.status.value if isinstance(topic.status, TopicStatus) else str(topic.status)
                existing.payload_json = payload
            await session.commit()
            return topic
...
class SqlEpisodeRepository(SqlBase):
    ...
    async def list_for_pod(self, pod_id: PodId) -> list[Episode]:
        async with self._session() as session:
            result = await session.execute(
                select(EpisodeRow).where(EpisodeRow.pod_id == pod_id).order_by(EpisodeRow.number.desc())
            )
...
class SqlShortRepository(SqlBase):
    ...
    async def list_for_pod(self, pod_id: PodId) -> list[Short]:
        async with self._session() as session:
            result = await session.execute(
                select(ShortRow).where(ShortRow.pod_id == pod_id).order_by(ShortRow.created_at.desc())
            )
...
class SqlJobRepository(SqlBase):
    ...
    async def save(self, job: Job) -> Job:
        async with self._session() as session:
            ...
            kind = job.kind.value if isinstance(job.kind, JobKind) else str(job.kind)
