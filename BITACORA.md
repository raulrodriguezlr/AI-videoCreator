...
## 2026-04 — Doblaje Inteligente (Aislamiento con Demucs) y YouTube SEO

### Contexto
El doblaje con ElevenLabs STS aplicaba la conversión sobre la pista entera de audio extraída de Veo. Esto provocaba que cualquier efecto de sonido (SFX) o ruido ambiental (agua, pasos) e incluso las risas naturales de los personajes se procesaran como habla. ElevenLabs intentaba "hablar" esos ruidos, resultando en artefactos robóticos y voces distorsionadas (*creepy*).

### Decisión: Aislamiento de pistas con Demucs (Meta AI)
Se integró el modelo open-source **Demucs** (`htdemucs`) para separar el audio antes del doblaje.

Nuevo pipeline de audio por clip:
1. Extraer audio nativo de Veo.
2. Pasar el audio por Demucs → genera `vocals.wav` (voz/risas) y `sfx.wav` (ruido ambiente/pasos/música).
3. Aplicar ElevenLabs STS **exclusivamente** sobre `vocals.wav`.
4. Remezclar el audio convertido de STS con la pista `sfx.wav` original de Veo (al 70% de volumen) usando FFmpeg.
5. Inyectar la mezcla final en el clip de vídeo.

### Resolución de problemas (SoundFile en Windows)
- Demucs fallaba de forma silenciosa en la exportación de `.wav` en sistemas Windows debido a la falta de un backend adecuado para `torchaudio`.
- Solución: Se incluyó `SoundFile` en `requirements.txt`.
- Manejo de I/O: Demucs escribe sus barras de progreso en `stderr`, lo que confundía el bloque `try...except`. Se ajustó `subprocess.run` para ignorar stdout/stderr y usar puramente `returncode` para validación.

### Mejoras en Metadatos de YouTube
Se actualizó el prompt de `YoutubeMetadataGenerator` (Gemini) para obligar a que **todas** las descripciones generadas terminen con el hashtag `#youtubekids`, mejorando el SEO y categorización automática para el target infantil.
